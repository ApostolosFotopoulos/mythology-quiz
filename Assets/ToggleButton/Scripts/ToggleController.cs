...
public class ToggleController : MonoBehaviour {
	...
	void Start() {
        if (PlayerPrefs.HasKey(optionText.text)) {
            int optionState = PlayerPrefs.GetInt(optionText.text);
            if (optionState == 1) {
                isOn = true;
            }
            else {
	...
	public void ChangeState() {
        if (isOn) {
            PlayerPrefs.SetInt(optionText.text, 1);
        }
        else {
            PlayerPrefs.SetInt(optionText.text, 0);
        }
    }
